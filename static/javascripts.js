...
/***
 *	main.js für WEB.STUMPP.NAME
 *
 *	Copyright 2022 Michael Stumpp (michael@stumpp.name)
 *
 *	07.01.2024 completly new - bundling JS
 *	13.01.2024 login | logoff
 *	03.02.2024 status buttons are grey now
 */
...
const status = {
	...
	insideHost : '',
	insideClient : '',
	version : '0.3.25'		/* we do not really need this ! */
}

/* das sind die beiden Buttons oben links im Header */
const buttonGlobe = document.getElementById('btn-globe');
...
document.addEventListener("DOMContentLoaded", function(event) {
	...
	if (buttonGlobe) {
		setLucideStroke(buttonGlobe, 'yellow', 1);
		buttonGlobe.addEventListener('click', showIntranet);
		showIntranet();
	}
	if (buttonLogin) {
		setLucideStroke(buttonLogin, 'transparent', 1);
		if (document.getElementById('login')) {
			setLucideStroke(buttonLogin, 'yellow', 1);
			buttonLogin.addEventListener('click', toggleUser);
			showUserStatus();
		} else {
 ...
 * *********************************************************************
 * showUserStatus()
 * setUserStatus()		Setzen der Programmglobalen Variable 'status'
 * 						Umschalten des 'header' Buttons
 * toggleUser()			Anmelden oder Abmelden
 *
 * ********************************************************************/
...
function toggleUser() {
	...
	console.log ("toggleUser",status.userState,user.status(),status.userName,user.name());
	setUserStatus();
}
 ...
 * Intranet detectopn
 * *********************************************************************
 * showIntranet()			Anzeige des Intranet-Status im Header Button
 * setIntranetStatus()		Prüfen des Intranet-Status
 * isIntranet(checkAgain)	Abfrage des Status (mit oder ohne Prüfung)
 *
 * ********************************************************************/

function showIntranet() {
	setIntranetStatus('', false, '');
	checkIntranet(setIntranetStatus);
}

function setIntranetStatus(fqdn, inside, data='') {
	status.insideState = inside;
	status.insideHost = fqdn;
	status.insideClient = data;
	setLucideStroke(buttonGlobe, inside?'#6F9':'#888', inside?2:1); // was '#6F9':'#0AF'
}

function isIntranet(checkAgain = false) {
	if (checkAgain) showIntranet();
	return status.insideState;
}
...
function clickMe(event) {
	...
	//~ const attrStroke = event.target.getElementsByTagName("svg")[0].getAttributeNode("stroke");
	const attrStroke = element.getAttributeNode("stroke");
	if (attrStroke.value == '#6F9')
		 setLucideStroke(buttonLogin, '#C44', 1);
	else setLucideStroke(buttonLogin, '#6F9', 2);
}
...
/**	********************************************************************
 *	stumpp/hwn intranet check
 *	executed once on web page start
/**	********************************************************************
 *
 *	Bedingungen
 *		Das 'company'-Logo sollte zugreifbar sein (https://172.16.1.4/company.svg
 *		Lokale IP muss in 172.16.1.0/24 liegen (falls ip.php vorhanden ist) --> SPÄTER
 *
 */

function checkIntranet(callbackFunction) {
	checkServerByImage(callbackFunction, "172.16.1.4", "/company.svg");
	//~ checkServerByPhp(callbackFunction, "172.16.1.4", "/ip.php");
}

/** checkServerByImage
 *
 *	Prüft auf die Existenz der angegebenen Datei.
 *	Wird kein Dateipfad übergeben, wird auf 'company.svg' geprüft
 */
function checkServerByImage(callback, fqdn, imagepath="/company.svg") {
    let img = new Image();
...
/** checkServerByImage
 *
 *	Prüft auf die Aufrufbarkeit  der angegebenen Datei.
 *	Wird kein Dateipfad übergeben, wird auf 'ip.php' abgerufen
 *	Das Ergebnis des Aufrufs wird im drtten Parameter an die Callback-Routine übergeben.
 */
 
function checkServerByPhp(callback, fqdn, phpfile="/ip.php") {
	const url = 'https://' + fqdn + phpfile;
	const method = 'GET';
	...
	xhttp.onload = function() {
		...
		const result = success ? this.responseText : '';
		if (callback) callback(fqdn, success, result);
	}
...
function defaultImage(svg=false) {
	if (svg) return '<svg xmlns="http://www.w3.org/2000/svg" width="400" height="400"></svg>';
	return 'data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHdpZHRoPSI0MDAiIGhlaWdodD0iNDAwIj48L3N2Zz4='
	}

function urlPicsum(imageID, suffix='') { return  "https://picsum.photos/id/" + imageID + suffix; }	
	
function loadPicsumImageById(imageID, picsumID = undefined, imageSize=256) {
	if (! imageID) return;
	if (picsumID === undefined) picsumID = Math.floor(Math.random()*1085);
	const url = "https://picsum.photos/id/" + picsumID + "/" + imageSize
	const img = document.getElementById(imageID);
	if (! img) return;
	img.addEventListener("error", (error) => {
		error.target.setAttribute("src", defaultImage());
		error.target.setAttribute("alt", "ERROR at picsum ID #" + picsumID);
	});	
	img.setAttribute("src", url);
	img.setAttribute("alt", "picsum Image #" + picsumID);
	img.setAttribute("data-pid", picsumID);
	//~ img.removeEventListener("error");
}

/**
 * 	Setzen von Effecten für alle Images einer Klasse
 *
 **/

function imageEffectById(imageId, effectName) {
	const element = document.getElementById(imageId);
	imageEffect(element, effectName);
}
	
function imageEffectByClassName(imageClass, effectName) {
	const images = document.getElementsByClassName(imageClass);
	for (let i = 0; i < images.length; i++) {
		imageEffect(images[i], effectName);
	}
}
	
function imageEffect(imageElement, effectName) {
	let list = imageElement.classList;
	if (effectName.startsWith('opacity')) {
		list.remove('w3-opacity-off');
		list.remove('w3-opacity-min');
		list.remove('w3-opacity'    );
		list.remove('w3-opacity-max');
	} else if (effectName.startsWith('grayscale')) {
		list.remove('w3-grayscale-min');
		list.remove('w3-grayscale'    );
		list.remove('w3-grayscale-max');
	} else if (effectName.startsWith('sepia')) {
		list.remove('w3-sepia-min');
		list.remove('w3-sepia'    );
		list.remove('w3-sepia-max');
	}

	if ((effectName != 'grayscale-off') && (effectName != 'sepia-off')) {
		list.add('w3-' + effectName);
	}
}
